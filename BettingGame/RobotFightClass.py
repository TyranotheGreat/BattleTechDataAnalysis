...
class RobotFightConstants:
    def __init__(self):
        self.json_reference = load_all_json_files()
        self.mech_list = []
        self.mech_list2 = []
        for mechdef in self.json_reference.keys():
            if mechdef.startswith("mechdef_"):
                self.mech_list.append(mechdef)
        self.mech_list2 = self.mech_list.copy()
...
class Mech:
    ...
    def __init__(self, name, mechdef_id, json_reference):
                    ...
                    if inventory2["DamageLevel"] == "Functional" and inventory2["ComponentDefType"] == "Weapon":
                        ...
                        if weapon_ammo_type2 == "Flamer" and weapon_ammo_type == weapon_ammo_type2:
                            ...
                            weapon_ammo_capacity += \
                                self.json_reference["Ammo_AmmunitionBox_Generic_" + weapon_ammo_type2][
                                    "Capacity"]
                        # "AmmoCategory": "Flamer"
...
def before_battle_create_csv():
    # Create the csv for the attack history
    with open('BattleHistory.csv', 'w') as out_file_init:
        out_file_init.write(
            "weapon,ammo_type,damage,shots,hit_percent,attack_number,isHit,target_body_part,target_damage_taken,"
            "round,turn,attacking_mechdef_id,defending_mechdef_id,battle_id,battle_date")
        out_file_init.write("\n")


def is_battle_over(mech):
...
def attack_mech(mech):
    ...
    for inventory in mech.mech_data["inventory"]:
        if inventory["DamageLevel"] == "Functional" and inventory["ComponentDefType"] == "Weapon":
            ...
            weapon_name = mech.json_reference[inventory["ComponentDefID"]]["Description"]["UIName"]
            weapon_damage_per_shot = mech.json_reference[inventory["ComponentDefID"]]["Damage"]
            weapon_shots = mech.json_reference[inventory["ComponentDefID"]]["ShotsWhenFired"]
            if weapon_shots > 1:
                weapon_damage_display = str(weapon_damage_per_shot) + " (x" + str(weapon_shots) + ")"
            else:
                weapon_damage_display = str(weapon_damage_per_shot)
            weapon_ammo_type = mech.json_reference[inventory["ComponentDefID"]]["AmmoCategory"]
    ...
    while i == 0:
        ...
        try:
            ...
            if not attack_count_array.__len__():
                ...
                melee_damage = mech.json_reference[mech.mech_data["ChassisID"]]["MeleeDamage"]
                # print(melee_damage)
                        ...
                        if inventory["RemainingAmmo"] < 0:
                            ...
                            for inventory2 in mech.mech_data["inventory"]:
                                if inventory2["DamageLevel"] == "Functional" and inventory2[
                                    ...
                                    weapon_ammo_type2 = mech.json_reference[inventory2["ComponentDefID"]][
                                        "AmmoCategory"]
...
def determine_target_body_part(mech):
    i = 0
    body_part = ""
    while i == 0:
...
class Battle:
    ...
    def start_battle(self):
        self.mech1.battle_id = self.battle_id
        self.mech2.battle_id = self.battle_id
        self.mech1.battle_date = self.battle_date
        self.mech2.battle_date = self.battle_date
        ...
        while isBattleOver == 0:
            ...
            self.mech1.round = current_round
            self.mech2.round = current_round
            self.mech1.turn = 1
            self.mech2.turn = 1
            self.take_damage(self.mech2, attack_mech(self.mech1))
            isBattleOver = is_battle_over(self.mech2)
            if isBattleOver == 0:
                print()
                print(whose_turn_message.format(self.mech2.name))
                self.mech1.turn = 2
                self.mech2.turn = 2
                self.take_damage(self.mech1, attack_mech(self.mech2))
                isBattleOver = is_battle_over(self.mech1)
                current_round += 1
        if self.mech1.isLoser == 1:
            print(self.mech2.name + " is the winner!")
        elif self.mech2.isLoser == 1:
            print(self.mech1.name + " is the winner!")

    def take_damage(self, mech, attacks):
        # Calculate a hit or miss to a random body part
        for attack in attacks:
            mech_location = determine_target_body_part(mech)
            attack_damage = 0
                ...
                if location["Location"] == inventory["MountedLocation"]:
                    if location["DamageLevel"] != inventory["DamageLevel"]:
                        ...
                        if inventory["ComponentDefType"] == "AmmunitionBox":
                            weapon_ammo_type = mech.json_reference[inventory["ComponentDefID"]]["Description"]["Model"]
                                ...
                                if inventory2["DamageLevel"] == "Functional" and inventory2[
                                    ...
                                    if weapon_ammo_type != 'NotSet' and weapon_ammo_type == weapon_ammo_type2:
                                        inventory2["RemainingAmmo"] -= \
                                            mech.json_reference["Ammo_AmmunitionBox_Generic_" + weapon_ammo_type][
                                                "Capacity"]
                        print(weapon_destroyed_message.format(
                            mech.json_reference[inventory["ComponentDefID"]]["Description"]["UIName"],
                            inventory["MountedLocation"]))

    def after_battle_write_csv(self):
...
def generate_battle_history(number):
    before_battle_create_csv()
    constants = RobotFightConstants()
    for a in constants.mech_list:
        for b in constants.mech_list2:
            for _ in range(0, number):
                mech1 = Mech("Bob", a, constants.json_reference)  # random.choice(mech_list))
                mech2 = Mech("Fred", b, constants.json_reference)  # random.choice(mech_list))
                battle1 = Battle(mech1, mech2, constants.json_reference)
                battle1.start_battle()
                battle1.after_battle_write_csv()
